"""
Code for dimensionality reduction with LLE and training SVCs  
"""
import time
import numpy as np
...
for i in range(3):
    ...
    # Dimensionality reduction of training dataset with LLE
    lle = LocallyLinearEmbedding(n_components=2, n_neighbors=20, 
                                 random_state=42, method="standard")
    X_train_lle = lle.fit_transform(X_train)
    scaler = StandardScaler()
    X_train_lle_s = scaler.fit_transform(X_train_lle)
    
    # Save LLE embedding and scaler
    jl.dump(lle, "saved_classifiers/lle_" + data_title + 
            "_Nsim" + str(Nsim) + "Nd" + str(Nd) + ".pkl")
    jl.dump(scaler, "saved_classifiers/lle_scaler_" + data_title + 
            "_Nsim" + str(Nsim) + "Nd" + str(Nd) + ".pkl")
    
    # Training data visualization
    fig, ax = plt.subplots(figsize=(9, 6.5))
    plot_train_data(X_train_lle_s, y_train, data_title + r"$_{train}$" + " LLE")
    plt.grid("on")
    plt.xlabel(r"LLE1")
    plt.ylabel(r"LLE2")
    ...
    if i==0:
        ...
        grid_search = GridSearchCV(
            ...
            param_grid = {"C": [0.0001, 0.01, 0.1, 1, 10, 20, 40, 60, 80]},
            cv = 5)
    grid_search.fit(X_train_lle_s, y_train)
    print("======================================================")
    print("Optimal model" + " (" + data_title + "):",
          grid_search.best_estimator_)
    clf = grid_search.best_estimator_
    clf.fit(X_train_lle_s, y_train)
    print(clf._gamma)

    # Save classifier
    jl.dump(clf, "saved_classifiers/clf_lle_" + clf.kernel + "_" + data_title + 
            "_Nsim" + str(Nsim) + "Nd" + str(Nd) + ".pkl") 

    # Performance measure
    cv_score = cross_val_score(clf, X_train_lle_s, y_train, cv=5, scoring="accuracy")
    y_train_pred = cross_val_predict(clf, X_train_lle_s, y_train, cv=5)
    ...
    X_test_lle_s = scaler.transform(X_test_lle)
    y_test_pred = clf.predict(X_test_lle_s)

    # Testing data visualization
    plot_test_data(X_test_lle_s, y_test, data_title + r"$_{train}$" + " LLE")
    plt.show()
    ...
    plt.axis([-3, 3, -3, 3])
    if i==0:
        plt.savefig("figures/LLE_X_linear" + "_Nsim" + str(Nsim) + 
                    "Nd" + str(Nd) + ".png", dpi=300, bbox_inches="tight")
    elif i==1:
        plt.savefig("figures/LLE_W_rbf" + "_Nsim" + str(Nsim) + 
                    "Nd" + str(Nd) + ".png", dpi=300, bbox_inches="tight")
    else:
        plt.savefig("figures/LLE_WP_rbf" + "_Nsim" + str(Nsim) + 
                    "Nd" + str(Nd) + ".png", dpi=300, bbox_inches="tight")
    plt.show()
